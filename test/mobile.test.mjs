...
function mock() {
  const writes = [], reads = [], shares = [], saves = [], documentSaves = [], commands = [], alerts = [], customAlerts = [], toasts = [];
  let closedAlerts = 0;
  ...
  const documentPicker = {
    saveDocuments: async options => {
      documentSaves.push(options);
      return [{ uri: 'content://downloads/' + options.fileName, name: options.fileName, error: null }];
    }
  };
  ...
  const vd = { metro: { common: { ReactNative: RN, React, navigation }, findByStoreName: name => stores[name],
    findByProps: (...keys) => [http, share, saveDialog, documentPicker, alertManager, ...Object.values(stores)].find(obj => keys.every(k => k in obj)) },
  ...
  return { writes, reads, shares, saves, documentSaves, commands, alerts, customAlerts, toasts, stores, file, share, saveDialog, documentPicker, http, vd, context, adapter, switchAccount: id => { owner = id; }, removed: () => removed, closedAlerts: () => closedAlerts };
}
...
test('RNDocumentPicker fallback saves the existing local export when RNShare is missing', async () => {
  const m = mock(); delete m.share.open;
  assert.doesNotThrow(() => m.adapter.ensureReady());
  assert.equal(m.adapter.capabilities().share, true);
  assert.equal(m.adapter.shareBackend(), 'document-picker');
  ...
  assert.equal(m.documentSaves.length, 1);
  assert.equal(m.documentSaves[0].sourceUris[0], 'file:///data/documents/DM.json');
  assert.equal(m.documentSaves[0].fileName, 'DM.json');
  assert.equal(m.documentSaves[0].mimeType, 'application/json');
  const diagnostic = m.adapter.sharingDiagnostics();
  assert.match(diagnostic, /RNDocumentPicker: saveDocuments/);
...
test('export preflight still works when no external save backend exists', () => {
  const m = mock(); delete m.share.open; delete m.documentPicker.saveDocuments;
  assert.doesNotThrow(() => m.adapter.ensureReady());
...
test('/exportdm direct export uses Android document picker when RNShare is unavailable', async () => {
  const m = mock(); delete m.share.open;
  const plugin = vm.runInContext(bundle, m.context);
  plugin.onLoad();
  ...
  for (let i = 0; i < 40 && m.documentSaves.length === 0; i++) await new Promise(resolve => setTimeout(resolve, 5));
  assert.equal(m.http.calls.length, 1);
  assert.ok(m.writes.length >= 2);
  assert.equal(m.shares.length, 0);
  assert.ok(m.documentSaves.length >= 1);
  assert.ok(m.documentSaves.every(call => call.sourceUris[0].startsWith('file://')));
  plugin.onUnload();
});
